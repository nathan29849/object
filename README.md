...
# 4장 설계 품질과 트레이드 오프
- 객체지향 설계의 핵심은 **`책임`**
- 책임을 할당하는 작업이 응집도와 결합도 같은 **`설계 품질`**과 관련
- 훌륭한 설계란, 합리적인 비용 안에서 변경을 수용할 수 있는 구조를 만드는 것
- 적절한 비용 안에서 쉽게 변경할 수 있는 설계는 응집도가 높고 서로 느슨하게 결합돼 있는 요소로 구성
- 결합도와 응집도를 합리적 수준으로 유지할 수 있는 중요한 원칙
...
- 객체를 설계하기 위한 가장 기본적인 아이디어 : 변경의 정도에 따라 구현과 인터페이스를 분리하고, 외부에서는 인터페이스에만 의존하도록 관계를 조절

### 응집도와 결합도
- 응집도와 결합도는 변경과 관련된 것들이다.
- 응집도 : 모듈에 포함된 내부 요소들이 연관돼 있는 정도 (하나의 목적을 위해 긴밀히 협력하는가)
- 결합도 : 의존성의 정도를 나타내며, 다른 모듈에 얼마나 많은 지식을 갖고 있는지를 나타내는 척도
- 만약 코드가 변경에 강하게 저항하고 있다면, 구성 요소들의 응집도가 낮고, 요소들이 강하게 결합됐을 가능성 농후
- 응집도 판단
  - 하나의 변경을 수용하기 위해 모듈 전체가 변경되면, 응집도가 높은 것
  - 하나의 변경을 수용하기 위해 모듈 일부만 변경되면, 응집도가 낮은 것
  - 하나의 변경을 수용하기 위해 하나의 모듈만 변경되면, 응집도가 높은 것
  - 하나의 변경을 수용하기 위해 다수의 모듈이 변경되면, 응집도가 낮은 것
- 결합도 판단 (한 모듈이 변경되기 위해 다른 모듈의 변경을 요구하는 정도)
  - 하나의 모듈을 수정할 때, 얼마나 많은 모듈을 함께 수정해야 하는가
  - 내부 구현을 변경했을 때, 이것이 다른 모듈에 영향을 미친다면, 결합도가 높은 것
  - 퍼블릭 인터페이스를 수정했을 때만, 다른 모듈에 영향을 미친다면, 결합도가 낮은 것
  - 따라서 구현이 아닌 인터페이스에 의존하도록 코드를 작성해야 낮은 결합도를 얻을 수 있다. 
- 캡슐화의 정도가 응집도와 결합도에 영향을 미친다.
  - 캡슐화를 지킬수록, 응집도가 높아지고, 결합도는 낮아지게 된다.
  - 따라서 응집도와 결합도를 고려하기 이전에, 캡슐화를 먼저 고려하자

### 단일 책임 원칙
- 클래스는 단 한가지의 변경 이유만 가져야 한다.
- 주의할 점
  - 단일 책임 원칙이라는 맥락에서 '책임'이라는 말이 '변경의 이유'라는 의미로 사용됨
  - 단일 책임 원칙에서의 책임은 지금까지 살펴본 역할, 책임, 협력에서 이야기 하는 책임과는 다르며, 변경과 관련된 더 큰 개념을 가리킴

## 캡슐화의 진정한 의미
- 캡슐화란 단순히 데이터 캡슐화만을 의미하는게 아니다.
- 진정한 의미의 캡슐화란, 변경될 수 있는 어떤 것이라도 감추는 것을 의미한다.
...
- 객체가 내부에 어떤 상태를 가지고 그 상태를 어떻게 관리하는 가는 부가적인 문제다.
